...
void main(List<String> args) async {
  ...
  try {
    ...
    tz.setLocalLocation(tz.getLocation(timeZoneName.localizedName?.name?? 'Africa/Cairo')); // حاول تجيب المنطقة الزمنية الحقيقية، ولو فشل استخدم القاهرة كاحتياطي
  } catch (e) {
  ...
  selectNotificationStream.stream.listen((String? payload) {
    if (payload != null) {
      navigatorKey.currentState?.push(
        MaterialPageRoute(
          builder: (_) => AdhanScreen(payload: payload),
        ),
      );
    } else {
      navigatorKey.currentState?.push(
